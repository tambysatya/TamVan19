...
data SearchZone = SZ { _szUB :: Bound,
                       _szLB :: Bound,
                       _szVal :: (Int, Double),
                       _szDefiningPoints :: AB.Array Int [IORef Point]}
...
proj :: Int -> A.UArray Int Double -> [Double]
proj k perf = [perf A.! i | i <- [b1..bp], i /= k]
    where (b1,bp) = A.bounds perf
  
definingPointP :: Int -> SearchZone -> Point -> Bool
definingPointP k zone pt = perf A.! k == u A.! k && and (L.zipWith (<) (proj k perf) (proj k u))
...
subdivideP (yU,yI) pt k zone 
	| pointPerf pt A.! k == yI A.! k = pure False
	| otherwise =  and <$> (forM [i | i <- [1..p], i /= k, _szUB zone A.! i /= (yU A.! i) + 1 ] $ \i -> do
